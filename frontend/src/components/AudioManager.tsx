...
export function AudioManager(props: {
  ...
  const setAudioFromDownload = async (data: ArrayBuffer, mimeType: string) => {
    ...
    props.uploadFile(
      new File([data], `audio_${Date.now()}.` + mimeType.split("/")[1], {
        type: mimeType,
      })
    );
    setAudioData({
  ...
  const setAudioFromRecording = async (data: Blob) => {
    ...
    fileReader.onloadend = async () => {
      ...
      props.uploadFile(
        new File([data], `audio_${Date.now()}.` + data.type.split("/")[1], {
          type: data.type,
        })
      );
      setAudioData({
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center">
        <div className="flex flex-row space-x-4 py-2 w-full px-2 border-2 rounded-full">
          <UrlTile
            icon={<AnchorIcon />}
            text={"Từ URL"}
            onUrlUpdate={(e) => {
          ...
          <FileTile
            icon={<FolderIcon />}
            text={"Từ máy tính"}
            onFileUpdate={(decoded, blobUrl, mimeType, data) => {
              props.transcriber.onInputChange();
              props.uploadFile(
                new File(
                  [data],
                  `audio_${Date.now()}.` + mimeType.split("/")[1],
                  {
                    type: mimeType,
                  }
                )
        ...
        <>
          ...
          {props.transcriber.progressItems.length > 0 && (
            <div className="relative z-10 p-4 w-full">
              <label>Tải mô hình... (Chỉ tải một lần)</label>
...
function SettingsModal(props: {
  ...
  const models = {
    ...
    "huuquyet/PhoWhisper-large": [3139],
...
function VerticalBar() {
  return <div className="w-[1px] bg-slate-200" />;
}
...
function ProgressBar(props: { progress: string }) {
  return (
    <div className="w-full bg-gray-200 rounded-full h-1 mt-4 dark:bg-gray-700">
      <div
...
function UrlModal(props: {
  ...
  return (
    <Modal
      show={props.show}
      title={"Lấy file từ URL"}
      content={
        <>
          {"Nhập đường dẫn của file âm thanh."}
...
function FileTile(props: {
  ...
  elem.oninput = (event) => {
    ...
    reader.addEventListener("load", async (e) => {
      ...
      props.onFileUpdate(decoded, urlObj, mimeType, arrayBuffer);
    });
...
function Tile(props: {
  ...
  return (
    <button
      ...
      className="flex items-center justify-center px-4 rounded-full p-2 bg-blue text-slate-600 hover:text-green-700 hover:bg-green-100 transition-all duration-200"
    >
      <div className="w-7 h-7">{props.icon}</div>
      {props.text && (
        <div className="ml-2 break-text font-medium text-center text-md w-30">
          {props.text}
        </div>
      )}
    </button>
